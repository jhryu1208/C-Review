...
// 전역변수는 소속된 코드블럭이  없으므로 프로그램이 종료될때까지 메모리에 존재하게되고
...
함수명 : 함수의 이름을 의미한다. 함수명은 가급적이면 이 함수가 하는 역할을 정확하게
	     표시해 주는 것이 코드 가독성에 좋다.

인자 : 해당 함수를 사용할 때 넘겨줄 값이 있을 경우 인자를 활용한다.
...
int Sum(int a, int b)	// 이때 int는 반환타입
{
...
// 아래 함수처럼 인자를 넘겨받게 될 경우 이 인자는 이 함수 코드블럭의 지역에 속해있다고 판단한다.
...
// 함수의 디폴트인자 : 인자는 기본적으로 값을 넘겨받게 설계되어 있다.
//					   그런데, 인자에 기본값을 대입해두면 인자를 넘겨주지 않을 경우 기본 설정한 값이 되고
//					   인자를 넘겨줄 경우 넘겨받은 값으로 인자를 설정하는 기능이다.
//					   디폴트인자는 가장 오른쪽부터 지정해 주어야 한다.
...
// 디폴트 인자에서 함수의 오버로딩은 조심해야한다. 컴퓨터입장에서는 무엇을 인쇄해야할지 애매해짐.
// void Output(int iNum)
// {
// }
...
오버로딩의 조건 : 함수 이름이 무조건 같아야 한다.
				  단, 인자의 개수 혹은 타입이 달라야 오버로딩이 성립이 된다.
				  반환타입은 오버로딩에 영향을 주지 않는다.
*/

// 아래 함수는 결과값을 인자로 받아와서 결과를 반환해주었다.
// 이런 형태의 함수를 Call by Address라고 한다.
// 포인터 주소를 받아오면, 위의 전역변수에 의해 발생하는 오류와 같은 오류가 생기지않는다.
...
int main()
{
	// 함수를 호출할 때는 함수명 (인자로 넘겨줄 값);  의 형태로 호출하게 된다.
	...
	// 디폴트 인자인 iNum1에 10이 들어가게 되고, iNum2는 처음에 지정한 10이 출력된다. 
	Output(10);
	// 디폴트 인자 iNum1에 10이 들어가고, 지정인자인 iNum2에도 30이 들어가게 된다.
	Output(10, 30);	
	...
	(지역변수로 인해 생기는 오류)
	iNum1은 위의 코드블럭 안에 존재하는 변수이므로 저 코드블럭 안에서만 사용가능하다.
	*/
